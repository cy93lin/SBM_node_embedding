...
#    plt.legend(['NMI-New', 'NMI-SC', 'NMI-ABP', 'CCR-New', 'CCR-SC', 'CCR-ABP'], loc=0)
    plt.legend(['NMI-New', 'NMI-SC', 'CCR-New', 'CCR-SC'], loc=0)
    plt.xlabel(param)
    plt.plot([1.27, 1.27],[0,1],'k-')
    plt.ylim(0,1)
...
#    c_array = [0.4, 0.6, 0.8, 1.0, 1.5, 2.0, 4.0]  # alpha = clog(n)/n
    c_array = [0.6, 0.8, 1.0, 1.2, 1.3, 1.5, 2.0, 2.5, 3.0, 4.0]  # alpha = clog(n)/n
    ...
    for c,K,N,lambda_n in itertools.product(c_array, K_array, N_array, lambda_array):
        ...
        for rand in range(rand_tests):
            ...
            y_abp = multi_abp(G,r,lambda1, m, mp, 10, K)
...
########################################
# use the following script to create figures     
import SBMmodels as SBM
import networkx as nx
from sklearn.cluster import KMeans
from sklearn.cluster import SpectralClustering
